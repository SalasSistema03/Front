<template>
      ...
      <div class="col-12 col-md-10">
        <form @submit.prevent="obtenerUnidades" class="row g-2" autocomplete="off">
          ...
          <div class="col-md-4">
            <div class="dropdown">
              <button
                class="btn btn-outline-secondary dropdown-toggle w-100 shadow-sm text-start d-flex justify-content-between align-items-center btn-sm"
                type="button" data-bs-toggle="dropdown" data-bs-auto-close="outside">
                Opciones de Filtro
              </button>
              <div class="dropdown-menu p-3 shadow" style="min-width: 250px;">
                <div class="form-check mb-2">
                  <input class="form-check-input" type="checkbox" v-model="filtros.opciones" value="ACTIVO"
                    id="activos">
                  <label class="form-check-label small" for="activos">Activos</label>
                </div>
                <div class="form-check mb-2">
                  <input class="form-check-input" type="checkbox" v-model="filtros.opciones" value="INACTIVO"
                    id="inactivos">
                  <label class="form-check-label small" for="inactivos">Inactivos</label>
                </div>
                <hr class="my-2">
                <div class="form-check mb-1">
                  <input class="form-check-input" type="checkbox" v-model="filtros.opciones" value="L"
                    id="admInmobiliario">
                  <label class="form-check-label small" for="admInmobiliario">Adm Inmobiliario</label>
                </div>
                <div class="form-check mb-1">
                  <input class="form-check-input" type="checkbox" v-model="filtros.opciones" value="P"
                    id="admPropietario">
                  <label class="form-check-label small" for="admPropietario">Adm Propietario</label>
                </div>
                <div class="form-check mb-1">
                  <input class="form-check-input" type="checkbox" v-model="filtros.opciones" value="I"
                    id="admInquilino">
          ...
          <table class="table table-hover table-striped tabla-impuestos mb-0 align-middle">
            ...
            <tbody>
              ...
              <tr v-else-if="unidades.length === 0">
                <td colspan="9" class="text-center py-4 text-muted">No se encontraron unidades con los filtros
                  aplicados.</td>
              </tr>
              ...
              <tr v-else v-for="unidad in unidades" :key="unidad.id"
                ...
                <td class="text-center">
                  <i v-if="tieneObservaciones(unidad.casa)" class="bi bi-chat-text-fill text-info"
                    title="Tiene observaciones"></i>
                </td>
  ...
  <ModalCargarUnidad v-if="mostrarModal" :show="mostrarModal" :unidad="unidadSeleccionada"
    :padrones="padronesSeleccionados" :edificiosLista="edificios" @close="mostrarModal = false"
